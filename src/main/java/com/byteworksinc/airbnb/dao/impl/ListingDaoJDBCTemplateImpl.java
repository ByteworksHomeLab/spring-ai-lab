...
public class ListingDaoJDBCTemplateImpl implements ListingDao {
    ...
    public Listing save(Listing listing) {
        String query = """
                insert into listing (
                id, listing_url, scrape_id, source, last_searched, last_scraped, name, description, neighborhood_overview, picture_url,
                ...
                host_thumbnail_url, host_picture_url, host_neighbourhood, host_listings_count, host_total_listings_count, host_verifications, host_has_profile_pic, host_identity_verified, neighbourhood, latitude,
                ...
                review_scores_cleanliness,review_scores_checkin, review_scores_communication, review_scores_location, review_scores_value, requires_license, license, instant_bookable, calculated_host_listings_count, calculated_host_listings_count_entire_homes,
                calculated_host_listings_count_private_rooms, calculated_host_listings_count_shared_rooms, region_id, region_name, region_parent_id, region_parent_name, region_parent_parent_id, region_parent_parent_name, reviews_per_month
                )
                values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                """;
        ...
        if (out == 0) {
            log.error("Listing save failed with id=" + listing.getId());
        }
        return findById(listing.getId());
    }
